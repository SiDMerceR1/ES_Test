package com.elastic.elasticsearch.search.util;
...
//
	...
	public static SearchRequest buildSearchRequest(final String indexName, final String field, final Date date1,
			final Date date2) {
		try {
			final SearchSourceBuilder builder = new SearchSourceBuilder()
					.postFilter(getQueryBuilder(field, date1, date2));
...
//    }
	...
	private static QueryBuilder getQueryBuilder(final String field, final Date date1, final Date date2) {
		return QueryBuilders.rangeQuery(field).gte(date1).lte(date2);
	}
}
